...
Ray generateReflectionRay(Ray ray, HitInfo hitInfo)
{
    ...
    return Ray { ray.origin + (ray.t - 0.001f) * ray.direction, mirrored, std::numeric_limits<float>::max() };
}
...
Ray generatePassthroughRay(Ray ray, HitInfo hitInfo)
{
    ...
    glm::vec3 origin((ray.origin + ray.direction * (ray.t + 0.001f)) + FLT_EPSILON * ray.direction);
...
void renderRayTransparentComponent(RenderState& state, Ray ray, const HitInfo& hitInfo, glm::vec3& hitColor, int rayDepth)
{
    ...
    // ...
    hitColor = renderRay(state, r, rayDepth + 1) * (1.0f - hitInfo.material.transparency) + hitColor * hitInfo.material.transparency;
}
